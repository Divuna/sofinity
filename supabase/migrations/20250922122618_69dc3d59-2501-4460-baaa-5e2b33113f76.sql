...
BEGIN
    ...
    RAISE NOTICE 'Found EventLogs: %, AIRequests: %, Notifications: %, Campaigns: %', 
    ...
    RAISE NOTICE 'Remapped EventLogs: %, AIRequests: %, Notifications: %, Campaigns: %', 
        eventlogs_updated, airequests_updated, notifications_updated, campaigns_updated;
    ...
    DECLARE
        ...
        percentage_el NUMERIC;
        percentage_ar NUMERIC;
        percentage_n NUMERIC;
        percentage_c NUMERIC;
    BEGIN
        ...
        percentage_el := CASE WHEN total_eventlogs > 0 THEN ROUND((valid_eventlogs::NUMERIC/total_eventlogs::NUMERIC)*100, 2) ELSE 100 END;
        percentage_ar := CASE WHEN total_airequests > 0 THEN ROUND((valid_airequests::NUMERIC/total_airequests::NUMERIC)*100, 2) ELSE 100 END;
        percentage_n := CASE WHEN total_notifications > 0 THEN ROUND((valid_notifications::NUMERIC/total_notifications::NUMERIC)*100, 2) ELSE 100 END;
        percentage_c := CASE WHEN total_campaigns > 0 THEN ROUND((valid_campaigns::NUMERIC/total_campaigns::NUMERIC)*100, 2) ELSE 100 END;
        
        RAISE NOTICE '=== ONEMIL TEST SUITE VERIFICATION ===';
        RAISE NOTICE 'EventLogs: %/% valid (%% percent)', valid_eventlogs, total_eventlogs, percentage_el;
        RAISE NOTICE 'AIRequests: %/% valid (%% percent)', valid_airequests, total_airequests, percentage_ar;
        RAISE NOTICE 'Notifications: %/% valid (%% percent)', valid_notifications, total_notifications, percentage_n;
        RAISE NOTICE 'Campaigns: %/% valid (%% percent)', valid_campaigns, total_campaigns, percentage_c;
    END;
