...
BEGIN
    ...
    RAISE NOTICE 'Creating user with email: %', target_email;
    ...
    RAISE NOTICE 'Creating profile for user: %', new_user_id;
    ...
    RAISE NOTICE 'Counting references to old test user: %', old_test_user_id;
    ...
    RAISE NOTICE 'EventLogs to remap: %', eventlogs_updated;
    RAISE NOTICE 'AIRequests to remap: %', airequests_updated;
    RAISE NOTICE 'Notifications to remap: %', notifications_updated;
    RAISE NOTICE 'Campaigns to remap: %', campaigns_updated;
    RAISE NOTICE 'Total references to remap: %', total_references;
    
    -- Step 4: Remap all references from old test user to new user
    RAISE NOTICE 'Remapping references to new user: %', new_user_id;
    ...
    RAISE NOTICE 'Remapped EventLogs: %', eventlogs_updated;
    RAISE NOTICE 'Remapped AIRequests: %', airequests_updated;
    RAISE NOTICE 'Remapped Notifications: %', notifications_updated;
    RAISE NOTICE 'Remapped Campaigns: %', campaigns_updated;
    ...
    BEGIN
        ...
        RAISE NOTICE 'Orphaned EventLogs: %', orphaned_eventlogs;
        RAISE NOTICE 'Orphaned AIRequests: %', orphaned_airequests;
        RAISE NOTICE 'Orphaned Notifications: %', orphaned_notifications;
        RAISE NOTICE 'Orphaned Campaigns: %', orphaned_campaigns;
        RAISE NOTICE 'Total Orphaned: %', total_orphaned;
        
        IF total_orphaned = 0 THEN
            RAISE NOTICE 'SUCCESS: All FK references valid (100%% integrity)';
        ELSE
            RAISE NOTICE 'WARNING: % orphaned references exist', total_orphaned;
        END IF;
    END;
    ...
    BEGIN
        ...
        RAISE NOTICE 'EventLogs: % valid out of %', valid_eventlogs, total_eventlogs;
        RAISE NOTICE 'AIRequests: % valid out of %', valid_airequests, total_airequests;
        RAISE NOTICE 'Notifications: % valid out of %', valid_notifications, total_notifications;
        RAISE NOTICE 'Campaigns: % valid out of %', valid_campaigns, total_campaigns;
        
        -- Calculate and display percentages
        IF total_eventlogs > 0 THEN
            RAISE NOTICE 'EventLogs validity: %% percent', ROUND((valid_eventlogs::NUMERIC/total_eventlogs::NUMERIC)*100, 2);
        END IF;
        IF total_airequests > 0 THEN
            RAISE NOTICE 'AIRequests validity: %% percent', ROUND((valid_airequests::NUMERIC/total_airequests::NUMERIC)*100, 2);
        END IF;
        IF total_notifications > 0 THEN
            RAISE NOTICE 'Notifications validity: %% percent', ROUND((valid_notifications::NUMERIC/total_notifications::NUMERIC)*100, 2);
        END IF;
        IF total_campaigns > 0 THEN
            RAISE NOTICE 'Campaigns validity: %% percent', ROUND((valid_campaigns::NUMERIC/total_campaigns::NUMERIC)*100, 2);
        END IF;
    END;
